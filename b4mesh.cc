...
// Pointer to the oracle (consensus) app of node i
...
// Pointer of the b4mesh (blockgraph) app of node i
 ...
 *
 * Update 15 May: No longer call ResubmitTransaction() since transactions are 
 * not broaadcast and only generated by leader..
 */
 ...
 * This is purely for tracing purposes.
 */
...
/**
 * For leader to check if block can be created.
...
void B4Mesh::RegisterTransaction(string payload){
  ...
  TransactionsTreatment(t); // Add to local mempool
 ...
 *
 * Update: No more broadcasting transasction.
 */
 ...
 * Checks if transaction is already in mempool/blockgraph, and if there is space in mempool.
 ...
 *
 * Update 15 May: No longer call this since transactions are not broadcast and are only generated by leader..
 */
...
/**
 * Checks if there is space left in local mempool.
 ...
 * For each of the childless hashes, make a call to ChildlessHashTreatment().
 * After this, start the actual merge synchronisation procedure by calling SendBranchRequest().
 */
...
void B4Mesh::SendBranchRequest(){
  ...
  int request_count = 0; // Should this be placed within the for loop?
 ...
 * The ip that is passed here from BlockTreatment() is actually the ip of that Block's leader. 
 */
 ...
 *
 * Update 15 May: Change logic to ask for childless hashes of ALL nodes in current group instead.
 */
 ...
 * Note that a merge block can also be created to join 2 childless blocks even without an actual merge(topology change) taking place.
 */
 ...
 * "Transactions" in a merge block are actually the all childless hashes of the blocks
 * it has to join (including those from other nodes' branches.
 */
...
/**
 * Invoked by consensus module, to notify this (Blockgraph) module whic is the new leader that has
 * been elected. This also means a merge has to take place, thus call StartMerge().
 */
